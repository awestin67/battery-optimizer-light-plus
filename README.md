...
* **🚫 Zero Export (Skydd vid negativa priser):** Tar emot en dynamisk exportgräns (kW) från molnet. Används för att förhindra att systemet förlorar pengar på att sälja solel när spotpriset plus din nätnytta understiger noll. För AC-kopplade solceller (t.ex. SolarEdge) finns en färdig Blueprint och guide: [`docs/SOLAREDGE_ZERO_EXPORT_GUIDE.md`](docs/SOLAREDGE_ZERO_EXPORT_GUIDE.md).
...
*   **Moderna system & EMS Site Limits (Firmware >= 1.35.14, t.ex. eco 10):**
    Integrationen känner automatiskt av mjukvaruversionen på ditt batteri och utnyttjar Sonnens moderna Energy Management System (Site Limits). Istället för att tvinga batteriet till manuell effektstyrning sätter integrationen intelligenta dynamiska gränser mot elnätet och låter batteriets inbyggda elektronik sköta regleringen:
    *   **Permanent Egenförbrukning (Self-Consumption):** Batteriet stannar kvar i sitt naturliga automatikläge. Inga slitande driftlägesväxlingar fram och tillbaka mellan manuell styrning och automatik vid paus eller vila.
    *   **Blixtsnabb lokal effektvakt (Peak Shaving):** Importtaket mot elnätet sätts direkt i växelriktarens hårdvara. Om en stor förbrukare (t.ex. ugn, spis eller elbil) slås på kliver batteriet in på millisekunder via sina egna lokala strömsensorer och kapar effekttoppen autonomt – helt utan fördröjning från Home Assistant eller nätverket.
    *   **Solel tas tillvara i IDLE & styrs vid HOLD:** I normalläget (IDLE) fortsätter batteriet alltid att autonomt absorbera tillgängligt solelöverskott från taket upp till 100 % (utan att behöva mikro-styras över nätverket). När elpriset däremot når en pristopp och molnet sätter HOLD, fryses batteriet så att 100 % av solöverskottet prioriteras för export och säljs till dagens högsta spotpris.
    *   **Smart exportspärr vid minuspriser:** När elpriset är negativt (p_gcp_max_export_limit: 0) spärras nätexporten helt, och allt solöverskott styrs istället rakt in i batteriet för att ladda det gratis istället för att behöva stänga av solcellerna.
    *   **Inbyggd Failsafe Watchdog (10 minuter):** Varje gräns skickas med en automatisk tidsbegränsning. Om Home Assistant, nätverket eller internetuppkopplingen skulle brytas återgår batteriet automatiskt och mjukt till vanlig standarddrift utan att riskera att fastna i ett oönskat läge.
    *   **Skonsamt mot elektroniken:** Gränserna hanteras i växelriktarens arbetsminne (RAM), vilket minimerar slitage på flashminnet vid täta uppdateringar.
*   **Äldre system (< 1.35.14, t.ex. eco 8):** Detekteras automatiskt och styrs sömlöst via beprövad klassisk styrning (manuella setpoints och driftlägesväxling).
