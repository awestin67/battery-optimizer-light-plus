...
@pytest.mark.asyncio
async def test_solis_battery_apply_action_charge():
    ...
    # Fejkad _find_entity som returnerar rätt entity_id för rätt söknyckel
    ...
    with patch.object(battery, "_find_entity", side_effect=mock_find_entity):
        await battery.apply_action("CHARGE", 3.5)
...
@pytest.mark.asyncio
async def test_solis_battery_apply_action_discharge():
    ...
    battery = SolisModbusBattery(mock_hass, "solis_inv_1", "sensor.soc")

    async def mock_find_entity(domain, partial_key):
        if partial_key == "rc_force_charge_discharge":
            return "select.solis_rc_mode"
        if partial_key == "rc_force_discharge_power":
            return "number.solis_discharge_power"
        if partial_key == "rc_timeout":
            return "number.solis_rc_timeout"
        if partial_key == "battery_discharge_limit_power":
            return "number.solis_discharge_limit"
        return None

    with patch.object(battery, "_find_entity", side_effect=mock_find_entity):
        await battery.apply_action("DISCHARGE", 4.2)
        ...
        # Verifiera urladdningseffekt (4.2 kW -> 4200 W)
        mock_hass.services.async_call.assert_any_call(
            "number", "set_value",
...
@pytest.mark.asyncio
async def test_solis_battery_apply_action_hold():
    ...
    with patch.object(battery, "_find_entity", side_effect=mock_find_entity):
        ...
        # Verifiera urladdningsgränsen = 0 W
        mock_hass.services.async_call.assert_any_call(
            "number", "set_value",
...
@pytest.mark.asyncio
async def test_solis_battery_apply_action_idle():
    ...
    with patch.object(battery, "_find_entity", side_effect=mock_find_entity):
        ...
        # Verifiera återställning av max urladdning
        mock_hass.services.async_call.assert_any_call(
            "number", "set_value",
        ...
        # Verifiera återställning av läge
        mock_hass.services.async_call.assert_any_call(
            "select", "select_option",
            {"entity_id": "select.solis_rc_mode", "option": "None"},
            blocking=True
